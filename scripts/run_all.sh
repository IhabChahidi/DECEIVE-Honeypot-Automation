...
################################################################################
# run_all.sh
# Master script to automate the entire honeypot project from installation to reporting.
# This version includes a checkpoint system to resume from the last successful step
# and skips Splunk installation if it is already installed and running.
################################################################################
...
take_screenshot() {
    ...
    if command -v gnome-screenshot &> /dev/null; then
        ...
        log_status "⚠️ Screenshot tool not found! Install 'gnome-screenshot' or 'scrot'."
        return
    fi
...
if [ "$LAST_STEP" -lt 1 ]; then
    if $SPLUNK_INSTALLED && $SPLUNK_RUNNING; then
        ...
        ./install_deceive_splunk.sh || log_status "⚠️ Warning: Installation script encountered an error, but continuing..."
    fi
...
if [ "$LAST_STEP" -lt 2 ]; then
    ...
    ./run_deceive.sh || log_status "⚠️ Warning: Honeypot failed to start, but continuing..."
...
if [ "$LAST_STEP" -lt 3 ]; then
    ...
    ./simulate_attack.sh || log_status "⚠️ Warning: Attack simulation failed, but continuing..."
...
if [ "$LAST_STEP" -lt 4 ]; then
    ...
    ./monitor_splunk.sh || log_status "⚠️ Warning: Splunk monitoring failed, but continuing..."
...
if [ "$LAST_STEP" -lt 5 ]; then
    ...
    ./configure_splunk_alerts.sh || log_status "⚠️ Warning: Splunk alert configuration failed, but continuing..."
...
if [ "$LAST_STEP" -lt 6 ]; then
    ...
    python3 log_analyzer_ai.py ~/DECEIVE/deceive.log 3 || log_status "⚠️ Warning: AI Log Analysis failed, but continuing..."
...
if [ "$LAST_STEP" -lt 7 ]; then
    ...
    ./generate_report.sh || log_status "⚠️ Warning: Report generation failed, but continuing..."
